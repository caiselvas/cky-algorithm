...
class CFG:
	...
	def __init__(self, rules: dict[str, set[str]], start_symbol: str|None=None) -> None:
		"""
		...
		The symbols must be single characters (strings of length 1): uppercase for non-terminal symbols and lowercase or numbers for terminal symbols.
		...
		self.available_non_terminals = list(sorted(set("ABCDEFGHIJKLMNOPQRSTUVWXYZ") - self.non_terminals, reverse=True))
	...
	def find_symbols(self, start_symbol: str|None) -> tuple[set, set]:
		...
		"""
		non_terminals = set(sym for sym in self.rules.keys())
		...
		for lhs, rhs in self.rules.items():
			for production in rhs:
				if len(production) == 1:
					...
					if any(not self.is_non_terminal(sym) for sym in production):
						return False
	...
	def to_cnf(self) -> None:
		...
		# print("Created new start symbol:", self)
		self.remove_null_productions()
		# print("Removed null productions:", self)
		self.remove_unit_productions()
		# print("Removed unit productions:", self)
		self.__remove_unreachable_productions()
		# print("Removed unreachable productions:", self)
		self.convert_to_binary_and_terminal_productions()
		# print("Converted to binary and terminal productions:", self)
			...
			for nt in self.non_terminals:
				...
				for production in rhs:
					if production != self.EPSILON:
						...
						nullable_positions = [i for i, sym in enumerate(production) if sym in nullable]
						for i in range(1, len(nullable_positions) + 1):
							for combination in combinations(nullable_positions, i):
								new_production = ''.join(
									sym for idx, sym in enumerate(production) if idx not in combination
								)
								if new_production:
									new_rhs.add(new_production)
				new_rules[nt] = new_rhs

			if self.start_symbol in nullable:
				new_rules[self.start_symbol].add(self.EPSILON)
			else:
				self.terminals.remove(self.EPSILON) # Because the only way to derive epsilon is through the start symbol

			self.rules = new_rules

	def __get_nullable_symbols(self):
		"""
		Identifies all nullable symbols.
		A nullable symbol is a non-terminal symbol that can derive ε in any number of steps.
		This means that a symbol is nullable if it directly derives ε or if any of its productions is made up of only non-terminal symbols that either nullable or the original symbol.
		"""
	...
	def remove_unit_productions(self) -> None:
		...
		for nt in self.non_terminals:
			new_rules[nt] = {production for production in new_rules[nt] if not (len(production) == 1 and production in self.non_terminals)}

		# Restore the original start symbol rules
		new_rules[self.start_symbol] = self.rules[self.start_symbol]
	...
	def __remove_unreachable_productions(self) -> None:
		...
		self.rules = {lhs: rhs for lhs, rhs in self.rules.items() if lhs in reachable}
					...
					while len(production) > 2:		
						...
						for pair in production_pairs:
							if len(pair) == 1:
								...
								new_nt = self.get_or_create_non_terminal(set(pair), rules=new_rules)
								new_production += new_nt

								new_rules[new_nt] = set(pair)
			...
			for production in rhs:
				if len(production) == 2:
					...
					if self.is_terminal(production[0]) and self.is_terminal(production[1]):
						first_new_nt = self.get_or_create_non_terminal(set(production[0]), rules=new_rules)
						second_new_nt = self.get_or_create_non_terminal(set(production[1]), rules=new_rules)
						new_rhs.add(first_new_nt + second_new_nt)

						new_rules[first_new_nt] = set(production[0])
						new_rules[second_new_nt] = set(production[1])
					
					# Handle t-nt combinations
					elif self.is_terminal(production[0]) and self.is_non_terminal(production[1]):
						new_nt = self.get_or_create_non_terminal(set(production[0]), rules=new_rules)
						new_rhs.add(new_nt + production[1])

						new_rules[new_nt] = set(production[0])

					# Handle nt-t combinations
					elif self.is_non_terminal(production[0]) and self.is_terminal(production[1]):
						new_nt = self.get_or_create_non_terminal(set(production[1]), rules=new_rules)
						new_rhs.add(production[0] + new_nt)

						new_rules[new_nt] = set(production[1])
	...
	def get_or_create_non_terminal(self, rhs: set, rules: dict[str, set[str]]|None=None) -> str:
		"""
		...
		"""
		existing_nt = self.get_lhs(rhs, rules)
	...
	def get_lhs(self, produced_rhs: set, rules: dict[str, set[str]]|None=None) -> None|str:
		"""
		Returns the symbol exactly produces the given production.

		Parameters
		----------
		produced_rhs (set): The production to find the symbol for.
		rules (dict, optional): The rules of the grammar. If not provided, the rules of the current grammar are used.
	...
	def set_rules(self, rules: dict[str, set[str]], start_symbol: str|None=None) -> None:
		"""
		...
		start_symbol (str, optional): The start symbol of the grammar. 
			If not provided, the first symbol is inferred checking all the productions generated by each symbol.
		"""
		self.__init__(rules=rules, start_symbol=start_symbol)
	...
	def __str__(self) -> str:
		...
		The representation includes the rules, start symbol, terminal symbols, and non-terminal symbols, all sorted in an understandable way.
		...
		"""
		produced_by_start = {production for production in self.get_rhs(self.start_symbol) if production in self.non_terminals}
		non_terminals_order = [self.start_symbol] + list(*produced_by_start) + list(sorted(self.non_terminals - {self.start_symbol} - produced_by_start))
		terminals_string = ', '.join(sorted(self.terminals))
		non_terminals_string = ', '.join(non_terminals_order)
		dict_string = '\n'.join(f"\t{lhs} --> {' | '.join(sorted(rhs))}" for lhs, rhs in zip(non_terminals_order, [self.get_rhs(nt) for nt in non_terminals_order]))
