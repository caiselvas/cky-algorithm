...
class CFG:
	...
	def remove_null_productions(self) -> None:
		...
		if self.EPSILON in self.terminals:
			...
			if self.start_symbol in nullable:
				new_rules[self.start_symbol].append(self.EPSILON)
			else:
	...
	def __get_nullable_symbols(self):
		...
		"""
		nullable = set(symbol for symbol, productions in self.rules.items() if self.EPSILON in productions)
		...
		while changed:
			...
			for symbol, productions in self.rules.items():
				if symbol in nullable:
					continue
				for production in productions:
					if all(char in nullable or char == symbol for char in production):
						nullable.add(symbol)
						changed = True
						break
		return nullable
	...
	def generate_words(self, max_length: int) -> set:
		...
		if max_length == 0:
			return {self.EPSILON} if self.EPSILON in self.terminals else set()
		...
		while queue:
			...
			if all(symbol not in self.rules for symbol in current_string):
				if len(current_string) <= max_length:
	...
	def __str__(self) -> str:
		...
		"""
		produced_by_start = {production for production in self.rules[self.start_symbol] if production in self.non_terminals}
		non_terminals_order = [self.start_symbol] + [*produced_by_start] + list(sorted(self.non_terminals - {self.start_symbol} - produced_by_start))
