...
class CKY:
	...
	def __parse_deterministic(self, word: str, grammar: CFG, visualize: bool) -> tuple[bool, list]:
		...
		if (not word) or (word == grammar.EMPTY):
			if grammar.EMPTY in grammar.get_rhs(start_symbol):
				return (True, [(start_symbol, grammar.EMPTY)])
			else:
				return (False, [])
	...
	def __parse_probabilistic(self, word: str, grammar: CFG, round_probabilities, visualize: bool) -> tuple[bool, float, list]:
		...
		if (not word) or (word == grammar.EMPTY):
			if grammar.EMPTY in grammar.get_rhs(start_symbol):
				empty_prob = grammar.get_probability(start_symbol, grammar.EMPTY)
				return (True, empty_prob, [((start_symbol, grammar.EMPTY), empty_prob)])
			else:
				return (False, 0.0, [])
