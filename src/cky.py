...
class CKY:
	...
	def __parse_probabilistic(self, word: str, grammar: CFG) -> tuple[bool, float, list]:
		"""
		...
		tuple[bool, float, list]
			A tuple containing a boolean indicating whether the word is in the language generated by the grammar,
			the total probability of the word being generated by the grammar, and a list of tuples, each containing
			a possible parse tree and its probability.
		"""
		n = len(word)
		...
		if n == 0:
			prob = grammar.get_probability(start_symbol, grammar.EPSILON) if grammar.EPSILON in grammar(start_symbol) else 0.0
			return (prob > 0, prob, [(grammar.EPSILON, prob)]) if prob > 0 else (False, 0.0, [])
							...
							if len(production) == 2:
								...
								if B in left_cell and C in right_cell:
									...
									if prob > 0:
										...
										backpointers[span - 1][start][lhs].append((B, C, start + split, prob))
		...
		# Retrieve all possible parse trees and their probabilities
		parse_trees = []
		if result:
			def build_trees(symbol, i, j):
				if j - i == 1:
				...
				trees = []
				for B, C, split, prob in backpointers[j - i - 1][i][symbol]:
					left_trees = build_trees(B, i, split)
					right_trees = build_trees(C, split, j)
					for lt in left_trees:
						for rt in right_trees:
							combined_prob = prob * lt[-1] * rt[-1]
							trees.append((symbol, lt, rt, combined_prob))
				return trees
			
			parse_trees = build_trees(start_symbol, 0, n)

		# Extract trees and their probabilities
		trees_with_probabilities = [(tree, prob) for tree, _, _, prob in parse_trees]

		return result, result_prob, trees_with_probabilities
