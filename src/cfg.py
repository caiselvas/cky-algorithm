...
class CFG:
	...
	def __init__(self, rules: dict[str, set[str]]|None=None, start_symbol: str|None=None, probabilistic: bool=False, from_file: str|None=None, from_text: str|None=None) -> None:
		"""
		...
		from_text (str, optional): The grammar rules in string format. Default is None.

		Initializes the CKY parser with a given Context-Free Grammar (CFG).
		"""
		assert (rules is not None) or (from_file is not None) or (from_text is not None), \
			"Please provide the rules of the grammar as a dictionary, a file path to read the rules from, or the text containing the rules."
	...
	def read_rules_from_file_or_text(self, file_path: str|None=None, text: str|None=None) -> dict[str, set[str]] | dict[str, set[tuple[str, int|float]]]:
		...
		"""
		assert (file_path is None) != (text not None), "Please provide only one of the file path or the text containing the grammar rules."
