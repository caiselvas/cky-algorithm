...
class CFG:
	...
	def __init__(self, 
		...
		EMPTY (str, optional): The symbol to represent the empty string. Default is 'ϵ' and other possible values are 'λ' and 'ε'.
			All the lowercase symbols except EMPTY will be considered as terminal symbols.
			This parameter does not affect the correct interpretation of the empty string "" in the grammar rules.
		...
		assert EMPTY in ['ε', 'λ', 'ϵ'], "The symbol to represent the empty string must be one of 'ε', 'λ', or 'ϵ'."
		self.EMPTY = EMPTY
		...
		for lhs, rhs in self.rules.items():
			for production in rhs:
				if len(production) == 1:
					...
					if lhs != self.start_symbol and production == self.EMPTY:
						return False
	...
	def __remove_epsilon_productions(self) -> None:
		...
		for lhs, rhs in new_rules.items():
			if lhs != self.start_symbol and self.EMPTY in rhs:
				new_rules[lhs].remove(self.EMPTY)
	...
	def get_nullable_nonterminals(self) -> set:
		...
		"""
		nullable = set(lhs for lhs, rhs in self.rules.items() if self.EMPTY in rhs)
	...
	def generate_words(self, max_length: int, round_probabilities: bool=False) -> list:
		...
		if max_length == 0:
			return [self.EMPTY] if self.is_terminal(self.EMPTY) else []
