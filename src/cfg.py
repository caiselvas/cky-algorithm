...
class CFG:
	...
	def __init__(self, rules: dict[str, set[str]]|None=None, start_symbol: str|None=None, probabilistic: bool=False, from_file: str|None=None) -> None:
		"""
		...
		If the grammar is read from a file, the file must contain the rules in the following format:
		...
		Parameters
		----------
		rules (dict, optional): A dictionary of rules. 
		...
		from_file (str, optional): The path to the file containing the grammar rules in string format. Default is None.

		Initializes the CKY parser with a given Context-Free Grammar (CFG).
		"""
		assert (rules is not None) or (from_file is not None), "Please provide the rules of the grammar or the path to the file containing the rules."
		
		self.probabilistic = probabilistic

		if from_file is not None:
			rules = self.read_rules_from_file(from_file)
	...
	def read_rules_from_file(self, file_path: str) -> dict[str, set[str]] | dict[str, set[tuple[str, int|float]]]:
		"""
		Reads the grammar rules from a file and returns them in the correct format.
	...
	def set_rules(self, rules: dict[str, set[str]], start_symbol: str|None=None, probabilistic: bool=False, from_file: str|None=None) -> None:
		...
		from_file (str, optional): The path to the file containing the grammar rules in string format. Default is None.
		"""
		self.__init__(rules=rules, start_symbol=start_symbol, probabilistic=probabilistic, from_file=from_file)
