...
class CKY:
	...
	def parse(self, word: str, grammar: CFG|None=None, probabilistic: bool=False) -> bool:
		...
		if isinstance(grammar, CFG) and grammar.probabilistic:
	...
	def __parse_probabilistic(self, word: str, grammar: CFG) -> float:
		"""
		...
		for i, char in enumerate(word):
			for lhs, productions in grammar.get_rules().items():
				if char in productions:
					table[0][i][lhs] = grammar.get_probability(lhs, char)
